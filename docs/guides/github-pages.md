...
- **Runtime assets**: `build:prod` copies files that Vite can't bundle (loaded at runtime via `fetch`): `experiences/aricanga/src/dist/locales/*.json` and `experiences/aricanga/src/dist/<locale>/story.json`. inkjs is bundled by Vite via ESM import. If you add new runtime-loaded assets, update the copy step in `mise.toml`.
