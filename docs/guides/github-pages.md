...
- **Runtime assets**: `build:prod` copies files that Vite can't bundle (loaded at runtime via `fetch` or `<script>` without `type="module"`): `packages/framework/src/vendor/ink.js`, `experiences/aricanga/src/dist/locales/*.json`, and `experiences/aricanga/src/dist/<locale>/story.json`. If you add new runtime-loaded assets, update the copy step in `mise.toml`.
